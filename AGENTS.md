...
- Introduce a modern architecture:
  ...
  - Clear support for:
    ...
    - **v2**: guest + signed-in mode with Firestore and pluggable auth provider (default: Firebase Auth).
...
- Always design for and use **Svelte 5** and its idioms.
  ...
  - For LCARS layout primitives (e.g. `LcarsApp`, `LcarsFooterBar`), prefer snippet props (`Snippet` + `{@render ...}`) over legacy slots / `<svelte:fragment>`.
  - When using TypeScript, avoid `any` and implicit `any`; always annotate function parameters (including snippet parameters and event handlers) with concrete types.
  - Prefer **pure functions** for domain logic, selectors, and helpers; keep side effects in the boundaries (stores, persistence, UI) so code remains easy to test.
  - When writing unit tests, avoid calling Svelte DOM lifecycle APIs (e.g. `mount` via `@testing-library/svelte` render) until tests are configured to run in a true browser client context; prefer store/utility behavior tests first.
...
- **Versioned behavior roadmap**
  - Define and maintain the versioned behavior model:
    - **v1 (Svelte initial release)**
      ...
      - LCARS shell, flows, canonical shortcuts, and import/export implemented in Svelte.
    - **v2 (Cloud sync + auth)**
      - Add authentication via an `AuthProvider` port.
        - Default implementation: `FirebaseAuthProvider` (Firebase Auth).
        - Auth usage must remain decoupled so the implementation can be swapped (e.g., Logto) without touching domain logic and UI flows.
...
- **Ports & abstractions**
  ...
  - Define an **auth port** (`AuthProvider`):
    - Methods like `signIn`, `signOut`, `onAuthStateChanged`, `getCurrentUser`.
    - Default implementation: wraps Firebase Auth (`FirebaseAuthProvider`).
    - CI and test usage:
      - Use configured **test users** (no separate machine-to-machine OAuth).
    - Rule:
      - No direct imports of Firebase Auth outside `src/lib/auth/`.
...
- **Observability**
  - Specify where and how to use **OpenTelemetry** (or equivalent):
    ...
    - Default exporter:
      ...
      - Optional OTLP exporter (e.g. Honeycomb), controlled via environment variables.
...
- **Non-negotiable behavior & UX**
  - Preserve core flows:
    ...
    - System reset.
  - Preserve canonical keyboard shortcuts (subject to Svelte/SPA constraints):
    - `Alt+H` (Home), `Alt+S` (Settings), `Alt+B` (Add Bookmark), `Alt+C` (Add Category), `Esc`, `Enter`.
...
- **Versioned behavior definition**
  ...
  - **v2 (guest + signed-in modes)**:
    - Add sign-in flows via `AuthProvider` (default: Firebase Auth).
      - Define supported auth methods (e.g. email+password, Google).
    ...
    - Define **mode semantics**:
      ...
      - On login:
        ...
        - Guest local data is not automatically merged; product may define an explicit “Import guest data” flow if desired.
...
- **Behavior parity & flows**
  ...
  - Implement canonical keyboard shortcuts:
    - `Alt+H` (Home), `Alt+S` (Settings), `Alt+B` (Add Bookmark), `Alt+C` (Add Category), `Esc`, `Enter`.
...
- **Auth & persistence wiring (v2)**
  ...
  - Ensure:
    ...
    - No direct Firebase Auth usage outside the auth adapter.
...
3. **Design primitives stay canonical (Svelte)**  
   - `UI-CSS-MAP.md` is the canonical public UI/CSS contract for LCARS primitives in the Svelte app.
   - Svelte LCARS components **wrap** those primitives:
     - They must not silently diverge from documented classes or semantics.
     - Any intentional divergence must be documented and reflected in `UI-CSS-MAP.md`.
   - `DESIGN.md` is legacy/reference unless explicitly updated for Svelte.

4. **Auth is port-only**  
   - All authentication must go through `AuthProvider`.
   - No direct imports of Firebase Auth (or any auth SDK) outside `src/lib/auth/`.
   - The default implementation is `FirebaseAuthProvider`, but the port must support swapping to Logto later.
...
6. **Spec- and test-driven changes**  
...
- **Persistence & auth**
  ...
  - v2:
    ...
    - Auth via `AuthProvider` port.
      - Default implementation: Firebase Auth (`FirebaseAuthProvider`).
      - The implementation must remain swappable (e.g. Logto).
...
- Documentation  
  ...
  - `UI-CSS-MAP.md` – Canonical LCARS primitive → CSS contract for the Svelte app.  
  - `DESIGN.md` – Legacy/reference design doc unless explicitly updated for Svelte.  
